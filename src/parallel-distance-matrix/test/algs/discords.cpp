...
ts_index_t n; // Length of discord
...
ts_element_t * generateRandomSinTimeSeries(long _discordLength)
{
	...
	assert(discord_loc >= 0 && discord_loc <= TIME_SERIES_LEN - n);
...
void SetParameters(void)
{
	...
	// Generate time series with random discord's starting point
		srand(time(NULL));
		discord_loc = rand() % (TIME_SERIES_LEN - n);
